...
public class JobSeekerController {
	...
	private ResponseEntity<?> registerJobSeeker(@RequestBody JobSeekerRequestDTO jobSeekerRequestDTO) {
	    log.info("Received JobSeeker registration request: {}", jobSeekerRequestDTO);
	    ResponseEntity<?> response;
	    try {
	        response = ResponseEntity.status(HttpStatus.CREATED).body(jobSeekerService.registerJobSeeker(jobSeekerRequestDTO));
	        log.info("JobSeeker registration successful.");
	    } catch (Exception e) {
	        log.error("Error occurred during JobSeeker registration: ", e);
	        throw e;
	    }
	    return response;
	}
